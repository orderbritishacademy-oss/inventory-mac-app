...
class LedgerWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        ttk.Button(mid, text="Download PDF", style="Refresh.TButton",
                   command=self.download_pdf).pack(side=tk.LEFT, padx=6)

        # --------------------- TREEVIEW WITH PAYMENT TYPE ---------------------
        columns = ("date", "type", "invoice", "credit", "debit",
                   "payment_type", "remaining", "amount")
        ...
        self.total_label = tk.Label(
            self, text="Total Amount: 0", font=("Arial", 12, "bold"),
            bg="#E8EAF6", fg="black"
        )
        self.total_label.pack(fill=tk.X, pady=5)

        self.remaining_label = tk.Label(
            self, text="Remaining Balance: 0", font=("Arial", 12, "bold"),
            bg="#E8EAF6", fg="blue"
        )
        self.remaining_label.pack(fill=tk.X, pady=5)
    ...
    def refresh_all(self):
        self.load_parties()     # refresh party list
        self.show_party()       # refresh table and totals

    # ---------------------------------------------------------------
    # GET TOTAL AMOUNT (sum of unique invoices)
    # ---------------------------------------------------------------
    def get_party_total(self, party):
    ...
    # ---------------------------------------------------------------
    def get_party_remaining(self, party):
        ledger = load_json(LEDGER_FILE)
        ent = ledger.get(party, {})
        ...
        try:
            return float(trans[-1].get("remaining", 0))
        except:
            return 0
    ...
    # ---------------------------------------------------------------
    # ADD POPUP (WITH PAYMENT TYPE COMBOBOX)
    # ---------------------------------------------------------------
    def open_add_popup(self):
        ...
        popup.geometry("440x460+100+60")
        popup.grab_set()

        labels = ["Date", "Type", "Invoice", "Credit", "Debit",
                  "Payment Type", "Remaining", "Amount"]
        self.entries = {}
        ...
        total_amount = self.get_party_total(party)
        remaining_balance = self.get_party_remaining(party)
        ...
        def update_remaining(event=None):
            ...
            new_remaining = remaining_balance - credit_val - debit_val

            self.entries["remaining"].config(state="normal")
            self.entries["remaining"].delete(0, tk.END)
            self.entries["remaining"].insert(0, str(new_remaining))
        ...
        for i, label in enumerate(labels):
            ...
            if label == "Payment Type":
                pay_options = ["CASH", "ONLINE", "CHEQUE", "UPI", "BANK TRANSFER"]
                combo = ttk.Combobox(popup, values=pay_options, state="readonly", width=27)
                combo.place(x=160, y=20 + i*40)
                combo.set("CASH")
                self.entries[label.lower()] = combo
                continue
            # -----------------------------------------------------------
            ...
            if label == "Date":
                ...
                ent.insert(0, str(total_amount))
                ent.config(state="readonly")

            elif label == "Remaining":
                invoice_list = [
                    t.get("invoice", "")
                    for t in load_json(LEDGER_FILE).get(party, {}).get("transactions", [])
                ]

                if len(set(invoice_list)) == len(invoice_list):
                    ent.insert(0, str(total_amount))
                else:
                    ent.insert(0, str(remaining_balance))
        ...
        ttk.Button(
            popup, text="Save", style="Refresh.TButton",
    ...
    def save_new_entry(self, party, popup):
        ...
        new_txn = {
            ...
            "credit": self.entries["credit"].get(),
            "debit": self.entries["debit"].get(),
            "payment_type": self.entries["payment type"].get(),
    ...
    def load_parties(self):
        ledger = load_json(LEDGER_FILE)
        self.party_cb["values"] = sorted(list(ledger.keys()))

    # ---------------------------------------------------------------
    def delete_row(self):
        party = self.party_cb.get()
        if not party:
            return messagebox.showwarning("Select", "Select a party first.", parent=self)

        selected = self.tree.focus()
        if not selected:
            return messagebox.showwarning("Select", "Select a row to delete.", parent=self)
        ...
        if party in ledger:
            ledger[party]["transactions"] = [
                txn for txn in ledger[party]["transactions"]
                if not (txn["date"] == row_date and txn["invoice"] == row_invoice)
            ]
            save_json(LEDGER_FILE, ledger)
    ...
    def show_party(self):
        ...
        ledger = load_json(LEDGER_FILE)
        ent = ledger.get(party, {})
        transactions = ent.get("transactions", [])
        ...
        for t in transactions:
            ...
            try:
                last_remaining = float(t.get("remaining", 0))
            except:
                last_remaining = 0

        invoice_list = [t.get("invoice", "") for t in transactions]
        if len(set(invoice_list)) == len(invoice_list):
            last_remaining = total_amount

        self.total_label.config(text=f"Total Amount: {total_amount}")
        self.remaining_label.config(text=f"Remaining Balance: {last_remaining}")
    ...
    def download_pdf(self):
        ...
        safe_name = "".join(c for c in party if c.isalnum() or c in (" ", "_", "-"))
        file_path = os.path.join(folder, f"{safe_name}.pdf")

        ledger = load_json(LEDGER_FILE)
        transactions = ledger.get(party, {}).get("transactions", [])
        ...
        def draw_col_text(c, x, width, y, text, align="left"):
            if align == "right":
                c.drawRightString(x + width - 5, y, str(text))
            else:
                c.drawString(x + 5, y, str(text))

        c.setFont("Helvetica-Bold", 18)
        c.drawString(40, y, f"Party Ledger Report: {party}")
        y -= 35

        c.setFont("Helvetica-Bold", 12)
        draw_col_text(c, *COLS["date"], y, "Date")
        draw_col_text(c, *COLS["type"], y, "Type")
        draw_col_text(c, *COLS["invoice"], y, "Invoice")
        draw_col_text(c, *COLS["credit"], y, "Credit")
        draw_col_text(c, *COLS["debit"], y, "Debit")
        draw_col_text(c, *COLS["payment_type"], y, "Pay Type")
        draw_col_text(c, *COLS["remaining"], y, "Remaining")
        draw_col_text(c, *COLS["amount"], y, "Amount")
        ...
        for t in transactions:
            ...
            cr = t.get("credit", "")
            db = t.get("debit", "")
            ptype = t.get("payment_type", "")
            rem = t.get("remaining", "")
            amt = t.get("amount", "")

            draw_col_text(c, *COLS["date"], y, date)
            draw_col_text(c, *COLS["type"], y, type_)
            draw_col_text(c, *COLS["invoice"], y, inv)
            draw_col_text(c, *COLS["credit"], y, cr, "right")
            draw_col_text(c, *COLS["debit"], y, db, "right")
            draw_col_text(c, *COLS["payment_type"], y, ptype)
            draw_col_text(c, *COLS["remaining"], y, rem, "right")
            draw_col_text(c, *COLS["amount"], y, amt, "right")
        ...
        y -= 30
        c.setFont("Helvetica-Bold", 14)
        c.drawString(40, y, f"Total Amount: {total_amount}")

        last_remaining = transactions[-1]["remaining"] if transactions else 0
        y -= 20
        c.drawString(40, y, f"Remaining Balance: {last_remaining}")
